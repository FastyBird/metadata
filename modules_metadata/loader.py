...
JSON_SCHEMAS_MAPPING = {
    ...
    ModuleOrigin(ModuleOrigin.MODULE_ACCOUNTS_ORIGIN).value: {
    ...
    ModuleOrigin(ModuleOrigin.MODULE_DEVICES_ORIGIN).value: {
    ...
    ModuleOrigin(ModuleOrigin.MODULE_TRIGGERS_ORIGIN).value: {
...
def load_schema(origin: ModuleOrigin, routing_key: RoutingKey) -> str:
    if origin.value in JSON_SCHEMAS_MAPPING:
        ...
        if routing_key.value in mapping:
            ...
            data = pkgutil.get_data("modules_metadata", "schema")

    elif routing_key.value in JSON_SCHEMAS_MAPPING[ModuleOrigin(ModuleOrigin.NOT_SPECIFIED_ORIGIN).value]:
